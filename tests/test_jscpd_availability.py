...
def test_jscpd_binary_exists():
    ...
    assert jscpd_bin.exists(), "jscpd binary not found - run 'cd red-agent && npm install'"
...
def test_package_lock_json_exists():
    ...
    assert package_lock.exists(), "red-agent/package-lock.json not found - required for security"
